...
export function PdfSplit() {
    ...
    if (!file) {
        return <Dropzone accept="application/pdf" onFile={handleFile} hint="Drop a PDF to extract pages" />;
    }
            ...
            <ToolPanel>
                <div className="flex flex-col gap-6 md:flex-row">
                    <div className="flex flex-1 flex-col items-center justify-center rounded-lg border border-border bg-card p-8 text-center">
                        ...
                        <GhostButton onClick={() => setFile(null)} className="mt-4">Choose another</GhostButton>
                    </div>

                    <div className="flex w-full flex-col gap-4 md:w-80">
                        <div>
                            <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Page Range</label>
                            <input
                                type="text"
                                value={range}
                            ...
                            />
                            <p className="mt-1.5 text-[10px] text-muted-foreground">Use commas for separate pages and dashes for ranges.</p>
                        </div>

                        <div className="space-y-2 pt-2">
                            <PrimaryButton onClick={splitPdf} className="w-full" disabled={processing}>
                                <Scissors className="mr-2 h-4 w-4" />
                                {processing ? "Extracting..." : "Extract Pages"}
                            </PrimaryButton>

                            <GhostButton onClick={extractAllPages} className="w-full" disabled={processing}>
