...
export function PdfCompress() {
    ...
    if (!file && !processing) {
        return <Dropzone accept="application/pdf" onFile={compressPdf} hint="Drop a PDF to compress it" />;
    }
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center">
                    <div className="mb-6 flex items-center justify-center gap-8">
                        <div className="flex flex-col items-center">
                            ...
                            <span className="text-sm font-semibold">{(stats?.original ? stats.original / 1024 : 0).toFixed(1)} KB</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center">
                            ...
                            <span className="text-xs font-medium uppercase text-muted-foreground">Compressed</span>
                            <span className="text-sm font-semibold">{(stats?.compressed ? stats.compressed / 1024 : 0).toFixed(1)} KB</span>
                        </div>
                    </div>
                    ...
                    <div className="mb-8">
                        ...
                        {stats && (
                            <p className="mt-1 text-sm text-accent font-medium">
                                Saved {((1 - stats.compressed / stats.original) * 100).toFixed(1)}% of file size
                            </p>
                        )}
                    </div>

                    <div className="flex gap-3">
                        <PrimaryButton onClick={() => file && compressPdf(file)} disabled={processing}>
                            {processing ? "Compressing..." : "Download again"}
                        </PrimaryButton>
                        <GhostButton onClick={() => { setFile(null); setStats(null); }}>Choose another</GhostButton>
                    </div>
                </div>
            </ToolPanel>
        </div>
