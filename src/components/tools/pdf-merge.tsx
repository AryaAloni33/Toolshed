...
export function PdfMerge() {
    ...
    const mergePdfs = async () => {
        ...
        try {
            ...
            console.error(error);
            toast.error("Failed to merge PDFs. One or more files might be corrupted.", { id: loadingToast });
        } finally {
            setProcessing(false);
        }
    };
    ...
    return (
        <div className="space-y-4">
            <ToolPanel>
                <div
                    ...
                    className={cn(
                        ...
                        files.length === 0 ? "items-center justify-center border-border bg-card hover:border-foreground/30" : "border-border bg-background"
                    )}
                >
                    {files.length === 0 ? (
                        <div className="text-center">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">Select multiple files to merge them</p>
                            <input
                                type="file"
                                multiple
                        ...
                        <div className="w-full space-y-2">
                            <div className="mb-4 flex items-center justify-between">
                                <h3 className="text-sm font-semibold">{files.length} files selected</h3>
                            ...
                            <div className="mt-6 flex flex-col gap-2">
                                <PrimaryButton onClick={mergePdfs} disabled={processing || files.length < 2}>
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-4 text-xs text-muted-foreground">
                <p><strong>Note:</strong> Merging happens entirely in your browser. Your files are never uploaded to a server.</p>
            </div>
        </div>
    );
}
