...
export function ImagePalette() {
    ...
    if (!src) {
        return <Dropzone accept="image/*" onFile={handleFile} hint="Extract the color palette from an image" />;
    }
            ...
            <ToolPanel>
                <div className="grid gap-8 md:grid-cols-[300px,1fr]">
                    <div className="overflow-hidden rounded-md border border-border bg-background">
                        <img src={src} alt="Source" className="w-full h-auto object-contain" />
                    </div>
                    ...
                    <div className="space-y-6">
                                ...
                                {colors.map((color, i) => (
                                    ...
                                    >
                                        ...
                                        <span className="font-mono text-[10px] text-muted-foreground uppercase flex items-center gap-1">
                                            {copied === color ? <Check className="h-2.5 w-2.5 text-moss" /> : null}
                                            {color}
                                        </span>
                                    </button>
                                ))}
                        ...
                        <div className="pt-4 flex gap-2">
                            <GhostButton onClick={() => setSrc(null)}>Choose another image</GhostButton>
                        </div>
                    </div>
                </div>
            </ToolPanel>
