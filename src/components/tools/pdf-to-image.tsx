...
export function PdfToImage() {
    ...
    if (!file) {
        return <Dropzone accept="application/pdf" onFile={handleFile} hint="Convert PDF pages to PNG images" />;
    }
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between border-b border-border pb-4">
                        <div>
                            ...
                            <p className="text-[10px] text-muted-foreground">Showing first {previews.length} pages</p>
                        </div>
                        <div className="flex gap-2">
                            <PrimaryButton onClick={convertAll} disabled={processing}>
                                {processing ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                                Download All PNGs
                            </PrimaryButton>
                            <GhostButton onClick={() => { setFile(null); setPreviews([]); }}>
                                Choose another
                            </GhostButton>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
                        {previews.map((src, i) => (
                            <div key={i} className="group relative aspect-[3/4] overflow-hidden rounded-md border border-border bg-muted/30">
                                <img src={src} alt={`Page ${i + 1}`} className="h-full w-full object-contain p-1" />
