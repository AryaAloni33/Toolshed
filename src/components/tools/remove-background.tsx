...
export function RemoveBackground() {
  ...
  const processImage = async (image: File | string) => {
    ...
    const loadingToast = toast.loading(
      "Removing background (loading AI model)...",
    );
    ...
    try {
      ...
      console.error(e);
      toast.error(
        "Failed to remove background. Your browser might not support the required AI features.",
        { id: loadingToast },
      );
    } finally {
      setProcessing(false);
  ...
  if (!src) {
    return (
      <Dropzone
        accept="image/*"
        onFile={handleFile}
        hint="Remove background using on-device AI"
      />
    );
  }
    ...
    <div className="space-y-4">
      <ToolPanel>
        ...
        <div className="mt-6 flex gap-3">
          {result && (
            <PrimaryButton
              onClick={() => result && saveAs(result, "no-bg.png")}
            >
      ...
      <div className="rounded-lg border border-border bg-muted/30 p-4 text-[10px] text-muted-foreground">
        <p>
          <strong>Note:</strong> This tool uses a ~80MB AI model that runs
          entirely in your browser. The first run might take a few moments to
          download the model.
        </p>
      </div>
    </div>
