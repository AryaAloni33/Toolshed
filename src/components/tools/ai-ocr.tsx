...
export function AiOcr() {
    ...
    const handleOcr = async () => {
        ...
        try {
            ...
            console.error(error);
            toast.error(error.message || "Something went wrong. Vision models might not be available or the image is unsupported.");
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-background p-8 transition-colors hover:border-foreground/20">
                    {image ? (
                        <div className="relative w-full max-w-sm">
                            <img src={image} alt="Upload preview" className="rounded-md shadow-sm border border-border" />
                            <button
                        ...
                        <label className="flex cursor-pointer flex-col items-center text-center">
                            ...
                            <span className="mt-1 text-xs text-muted-foreground">Supports JPG, PNG, WEBP (Max 4MB)</span>
