...
import { FileArchive, File as FileIcon, Download, FolderOpen } from "lucide-react";
...
export function UnzipTool() {
        ...
        try {
            for (const entry of entries) {
                if (!entry.dir) {
                    ...
                    if (content) saveAs(content, entry.name.split("/").pop() || entry.name);
                }
            }
    ...
    if (!file) {
        return <Dropzone accept=".zip" onFile={handleFile} hint="Drop a .zip file to extract its contents" />;
    }
                    ...
                    <div className="flex items-center justify-between border-b border-border pb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-muted-foreground">{entries.filter(e => !e.dir).length} files • {(file.size / 1024).toFixed(1)} KB</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <PrimaryButton onClick={downloadAll} disabled={processing} className="h-8 px-3 text-xs">
                                Extract All
                            </PrimaryButton>
                            <GhostButton onClick={() => { setFile(null); setEntries([]); setZipInstance(null); }} className="h-8 px-3 text-xs">
                                Close
                            </GhostButton>
                        </div>
                    </div>
                                ...
                                {entries.map((entry, i) => (
                                    <tr key={i} className="hover:bg-muted/30">
                                        <td className="flex items-center gap-2 px-4 py-2">
                                            {entry.dir ? <FolderOpen className="h-3.5 w-3.5 text-accent/60" /> : <FileIcon className="h-3.5 w-3.5 text-muted-foreground" />}
