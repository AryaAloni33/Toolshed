...
const transforms: { id: string; label: string; fn: (s: string) => string }[] = [
  ...
  { id: "camel", label: "camelCase", fn: (s) => s.toLowerCase().replace(/[^a-z0-9]+(.)/g, (_, c) => c.toUpperCase()) },
  { id: "snake", label: "snake_case", fn: (s) => s.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "") },
  { id: "kebab", label: "kebab-case", fn: (s) => s.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") },
  { id: "reverse", label: "esreveR", fn: (s) => s.split("").reverse().join("") },
];

export function CaseConverter() {
  const [input, setInput] = useState("Convert me — I'm waiting in the toolshed.");
        ...
        {transforms.map((t) => {
          ...
          return (
            <ToolPanel key={t.id}>
              ...
              <p className="break-words text-sm text-foreground/90">{out || "—"}</p>
            </ToolPanel>
          );
        })}
      </div>
